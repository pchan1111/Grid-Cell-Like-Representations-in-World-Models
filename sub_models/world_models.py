...
def norm_relu(x, eps=1e-12):
    x = F.relu(x)
    norm = torch.linalg.vector_norm(x, ord=2, dim=-1, keepdim=True)

    return x / torch.maximum(norm, torch.tensor(eps, device=x.device))
...
class GridCellLoss(nn.Module):
    def __init__(self, alpha=0.9, sigma=0.6): # small sigma to focus on close points (Default: 0.54, 1.2)
        super().__init__()
        self.alpha = alpha
        self.sigma = sigma

    def pairwise_JSD(self, logits, eps=1e-6):
    ...
    def pairwise_euclidian_distance(self, g_seq):
    ...
    def forward(self, g_seq, latents_logits_seq, anchor_g_seq):
        ...
        return total_loss, loss_dist, loss_cap, loss_anchor
...
class WorldModel(nn.Module):
    ...
    def update(self, obs, action, reward, termination, total_steps, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            grid_cell_loss, loss_dist, loss_cap, loss_anchor = self.grid_cell_loss(g_seq, post_logits, anchor_g_seq)
        ...
        if self.record_run:
            wandb.log({
                ...
                "WorldModel/1.6.loss_dist": loss_dist.item(),
                "WorldModel/1.7.loss_cap": loss_cap.item(),
                "WorldModel/1.8.loss_anchor": loss_anchor.item(),
            ...
            if logger is not None:
                ...
                logger.log("WorldModel/1.6.loss_dist", loss_dist.item())
                logger.log("WorldModel/1.7.loss_cap", loss_cap.item())
                logger.log("WorldModel/1.8.loss_anchor", loss_anchor.item())
