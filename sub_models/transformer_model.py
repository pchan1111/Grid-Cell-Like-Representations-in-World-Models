...
class StochasticTransformerKVCache(nn.Module):
    ...
    def reset_kv_cache_list(self, batch_size, dtype, max_cache_length=None):
        ...
        self.current_position = 0  # Track actual position independent of cache length
        self.key_cache_list = []
    ...
    def forward_with_kv_cache(self, samples, action):
        ...
        action = F.one_hot(action.long(), self.action_dim).float()
        feats = self.stem(torch.cat([samples, action], dim=-1))
        feats = self.position_encoding.forward_with_position(feats, position=self.current_position)
        feats = self.layer_norm(feats)

        for idx, layer in enumerate(self.layer_stack):
            feats, attn = layer.forward_with_kv_cache(feats, self.key_cache_list, self.value_cache_list, idx)

            if self.max_cache_length is not None and self.key_cache_list[idx].shape[2] > self.max_cache_length:
        ...
        # Increment position counter after processing
        self.current_position += 1
        if self.current_position >= self.max_len:
            self.reset_kv_cache_list(batch_size=samples.shape[0], dtype=samples.dtype, max_cache_length=self.max_cache_length)
        return feats