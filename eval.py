...
if __name__ == "__main__":
    ...
    for step in tqdm(steps):
        # Load state dict and remove _orig_mod. prefix added by torch.compile()
        world_model_state = torch.load(f"{root_path}/world_model_{step}.pth")
        world_model_state = {k.replace('_orig_mod.', ''): v for k, v in world_model_state.items()}
        world_model.load_state_dict(world_model_state)
        
        agent_state = torch.load(f"{root_path}/agent_{step}.pth")
